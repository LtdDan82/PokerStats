...
class PlayerStats(object):
   ...
   # wins = df_wins.loc[selection]
    ...
    def get_phase_raises(self, phase):
        ...
        df_raise_event = self.df_final[((self.df_final['gamephase'] == get_phase[0]) |\
    ...
    def cbet_flop(self):
        ...
        raised_cbets = pre_phase[(pre_phase['action'] == 'RAISE') |\
                                     (pre_phase['action'] == 'BET')]
        game_number = raised_cbets.groupby(['game_number'])[['action', 'player']].agg(list)        
        unique_game_number = list(set(game_number.index.tolist()))
        for num in unique_game_number:
            check_df = raised_cbets[raised_cbets['game_number'] == num].reset_index(drop=True)
            #todo
    def get_player_action(self):
        actions = self.df_final.query("action in ['RAISE', 'BET', 'CHECKS', 'FOLD', 'CALLS', 'WIN_POT']")
        player_action_count = pd.DataFrame(actions.groupby(['player', 'action'])['game_number'].count())
        player_action_count = player_action_count.rename(columns = {'game_number': 'counts'})
        df_pivot = pd.pivot_table(data = player_action_count,
